...
param alwaysUseShortLocation bool = false
...
var regularName = replace(
  replace(
    replace(
      replace(
        replace(
          replace(namingConventionSegmentSeparatorProcessed, '{env}', toLower(environment)),
          '{loc}',
          alwaysUseShortLocation ? shortLocationValue : location
        ),
        '{seq}',
        sequenceFormatted
      ),
