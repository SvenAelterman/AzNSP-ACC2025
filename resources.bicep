...
module nspModule 'nsp.bicep' = if (deployNsp) {
  name: 'nspDeployment'
  params: {
    ...
    associationResourceIds: associateNsp
      ? [
          sqlServerModule.outputs.resourceId
          keyVaultModule.outputs.resourceId
          logAnalyticsModule.outputs.resourceId
        ]
      : []
...
module nspNameModule 'common-modules/createValidAzResourceName.bicep' = {
  name: 'nspNameDeployment'
  params: {
    location: location
    environment: environment
    namingConvention: namingConvention
